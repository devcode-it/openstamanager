...
if (!empty($type) && $type != 'menu' && $type != 'custom') {
    ...
    echo '
    <table data-id_module="'.$id_module.'" data-id_plugin="'.$id_plugin.'" data-id_parent="'.$id_record.'" data-selected="'.implode(';', $selezione).'" id="'.$table_id.'" width="100%" class="table main-records table-hover table-striped '.(!empty($id_plugin) ? '-plugins' : '').'">
            <thead>
                <tr>
