...
<form action="" method="post" id="add-form">
	...
	<div class="row">
		...
		<div class="col-md-6">
			{[ "type": "select", "label": "'.tr('Tipo di anagrafica').'", "name": "id_tipo_anagrafica[]", "id": "id_tipo_anagrafica_add", "multiple": "1", "required": 1, "values": "query=SELECT `an_tipi_anagrafiche`.`id`, `title` as descrizione FROM `an_tipi_anagrafiche` LEFT JOIN `an_tipi_anagrafiche_lang` ON (`an_tipi_anagrafiche`.`id` = `an_tipi_anagrafiche_lang`.`id_record` AND `an_tipi_anagrafiche_lang`.`id_lang` = '.prepare(Models\Locale::getDefault()->id).') WHERE `an_tipi_anagrafiche`.`id` NOT IN (SELECT DISTINCT(`x`.`id_tipo_anagrafica`) FROM `an_tipi_anagrafiche_anagrafiche` x INNER JOIN `an_tipi_anagrafiche` t ON `x`.`id_tipo_anagrafica` = `t`.`id` LEFT JOIN `an_tipi_anagrafiche_lang` ON (`t`.`id` = `an_tipi_anagrafiche_lang`.`id_record` AND `an_tipi_anagrafiche_lang`.`id_lang` = '.prepare(Models\Locale::getDefault()->id).') INNER JOIN `an_anagrafiche` ON `an_anagrafiche`.`id` = `x`.`id_anagrafica` WHERE `an_tipi_anagrafiche`.`id` = '.prepare($id_tipo_azienda).' AND `deleted_at` IS NULL) ORDER BY `title`", "value": "'.($id_tipo_anagrafica ?? null).'", "readonly": '.(!empty(get('readonly_tipo')) ? 1 : 0).' ]}
		</div>
	</div>
