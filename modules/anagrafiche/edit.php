...
if ($is_cliente or $is_fornitore or $is_tecnico) {
    ...
    echo '
                        ...
                        <div class="tab-pane '.(!$is_cliente ? 'hide' : 'active').'" id="cliente">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    {[ "type": "select", "label": "'.tr('Agente principale').'", "name": "id_agente", "values": "query=SELECT `an_anagrafiche`.`id` AS id, IF(deleted_at IS NOT NULL, CONCAT(`ragione_sociale`, \' (Eliminato)\'), `ragione_sociale` ) AS descrizione FROM `an_anagrafiche` INNER JOIN (`an_tipianagrafiche_anagrafiche` INNER JOIN `an_tipianagrafiche` ON `an_tipianagrafiche_anagrafiche`.`id_tipo_anagrafica`=`an_tipianagrafiche`.`id` LEFT JOIN `an_tipianagrafiche_lang` ON (`an_tipianagrafiche_lang`.`id_record` = `an_tipianagrafiche`.`id` AND `an_tipianagrafiche_lang`.`id_lang` = '.prepare(Locale::getDefault()->id).')) ON `an_anagrafiche`.`id`=`an_tipianagrafiche_anagrafiche`.`id_anagrafica` WHERE (`title`=\'Agente\' AND `deleted_at` IS NULL)'.(isset($record['id_agente']) ? 'OR (`an_anagrafiche`.`id` = '.prepare($record['id_agente']).' AND `deleted_at` IS NOT NULL) ' : '').'ORDER BY `ragione_sociale`", "value": "$id_agente$" ]}
                                </div>
                            </div>
    ...
    // Collegamento con il conto
    $conto = $dbo->fetchOne('SELECT co_pianodeiconti3.id, co_pianodeiconti2.numero as numero, co_pianodeiconti3.numero as numero_conto, co_pianodeiconti3.descrizione AS descrizione FROM co_pianodeiconti3 INNER JOIN co_pianodeiconti2 ON co_pianodeiconti3.id_piano_dei_conti2=co_pianodeiconti2.id WHERE co_pianodeiconti3.id = '.prepare($record['id_conto_fornitore']));
