...
if (get('op') == 'get_costo_ore') {
    ...
    $rs = $dbo->fetchArray('SELECT `costo_ore` FROM `co_contratti_tipi_intervento` WHERE `id_contratto`='.prepare($id_record).' AND `id_tipo_intervento`='.prepare($id_tipo_intervento));
