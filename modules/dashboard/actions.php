...
switch (get('op')) {
    ...
    case 'get_more_info':
        ...
        if (!empty($rs)) {
            ...
            $query = 'SELECT *, in_interventi.codice, id_stato AS parent_id_stato, id_tipo_intervento AS parent_idtipo, (SELECT descrizione FROM in_statiintervento WHERE id=parent_id_stato) AS stato, (SELECT descrizione FROM in_tipiintervento WHERE id_tipo_intervento=parent_idtipo) AS tipo, (SELECT nomesede FROM an_sedi WHERE id=idsede) AS sede, (SELECT idzona FROM an_anagrafiche WHERE idanagrafica=in_interventi.idanagrafica) AS idzona FROM in_interventi LEFT JOIN an_anagrafiche ON in_interventi.idanagrafica=an_anagrafiche.idanagrafica WHERE in_interventi.id='.prepare($id).' '.Modules::getAdditionalsQuery('Interventi');
            $rs = $dbo->fetchArray($query);
    ...
    case 'load_intreventi':
        ...
        $interventi = $dbo->fetchArray("SELECT id, richiesta, DATE_FORMAT(data_richiesta,'%m%Y') AS mese, data_richiesta, an_anagrafiche.ragione_sociale, 'intervento' AS ref, (SELECT descrizione FROM in_tipiintervento WHERE in_tipiintervento.id=in_interventi.id_tipo_intervento) AS tipointervento FROM in_interventi INNER JOIN an_anagrafiche ON in_interventi.idanagrafica=an_anagrafiche.idanagrafica WHERE (SELECT COUNT(*) FROM in_interventi_tecnici WHERE in_interventi_tecnici.idintervento = in_interventi.id) = 0 ORDER BY data_richiesta ASC");
