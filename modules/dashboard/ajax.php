...
switch (filter('op')) {
    ...
    case 'interventi_periodo':
        ...
        foreach ($sessioni as $sessione) {
            ...
            $results[] = [
                ...
                'extendedProps' => [
                    'link' => base_path_osm().'/editor.php?id_module='.$modulo_interventi->id.'&id_record='.$sessione['id_intervento'],
                    'id_intervento' => $sessione['id_intervento'],
                ...
                if ($preventivo['is_pianificabile'] == 1 || $preventivo['stato'] = 'In attesa di conferma') {
                    if (!empty($preventivo['data_accettazione']) && $preventivo['data_accettazione'] != '0000-00-00') {
                        ...
                        $results[] = [
                            ...
                            'url' => base_path_osm().'/editor.php?id_module='.$modulo_preventivi->id.'&id_record='.$preventivo['id'],
    ...
    case 'modifica_intervento':
        ...
        $intervento = Intervento::find(filter('id_intervento'));
