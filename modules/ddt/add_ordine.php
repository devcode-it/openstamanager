...
<div class="row">
    <div class="col-md-12">
        {[ "type": "select", "label": "'.tr('Ordine').'", "name": "id_documento", "values": "query=SELECT `or_ordini`.`id`, CONCAT(IF(`numero_esterno` != \'\', `numero_esterno`, `numero`), \' del \', DATE_FORMAT(`data`, \'%d-%m-%Y\')) AS descrizione FROM `or_ordini` INNER JOIN `or_stati_ordine` ON `or_ordini`.`id_stato`=`or_stati_ordine`.`id` INNER JOIN `or_tipi_ordine` ON `or_ordini`.`id_tipo_ordine`=`or_tipi_ordine`.`id` LEFT JOIN `or_stati_ordine_lang` ON (`or_stati_ordine`.`id`=`or_stati_ordine_lang`.`id_record` AND `or_stati_ordine_lang`.`id_lang` = '.prepare(Models\Locale::getDefault()->id).') INNER JOIN `or_righe_ordini` ON `or_ordini`.`id` = `or_righe_ordini`.`id_ordine` WHERE `id_anagrafica`='.prepare($id_anagrafica).' AND `or_stati_ordine_lang`.`title` IN(\'Accettato\', \'Evaso\', \'Parzialmente evaso\', \'Parzialmente fatturato\') AND `dir`='.prepare($dir).' AND (`or_righe_ordini`.`qta` - `or_righe_ordini`.`qta_evasa`) > 0 GROUP BY `or_ordini`.`id` ORDER BY `or_ordini`.`id` DESC, `data` DESC, `numero` DESC" ]}
    </div>
</div>
