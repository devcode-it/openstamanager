...
$righe = $dbo->fetchArray(
    ...
    FROM 
        `dt_righe_ddt`
        INNER JOIN `mg_articoli` ON `mg_articoli`.`id` = `dt_righe_ddt`.`id_articolo`
            ...
            <?php foreach ($righe as $riga) { ?>
                ...
                $ultimo_prezzo_preventivo = $dbo->fetchArray(
                    ...
                    FROM
                        ...
                        INNER JOIN `dt_righe_ddt` ON `dt_righe_ddt`.`id_articolo` = `mg_articoli`.`id`
                    WHERE
                        `co_preventivi`.`id_anagrafica` = '.prepare($id_anagrafica).' AND `dt_righe_ddt`.`id_articolo` = '.prepare($riga['id_articolo']).' AND `co_stati_preventivi_lang`.`title` NOT IN ("Bozza", "In attesa di conferma", "Rifiutato")
                    GROUP BY
