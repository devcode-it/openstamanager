...
$righe = $dbo->fetchArray(
    'SELECT mg_articoli.descrizione, dt_righe_ddt.*
    FROM dt_righe_ddt
            ...
            <?php foreach ($righe as $riga) { ?>
                <?php
                    ...
                    $ultimo_prezzo_preventivo = $dbo->fetchArray(
                        'SELECT dt_righe_ddt.idarticolo, dt_righe_ddt.prezzo_unitario, date(co_preventivi.updated_at) as updated_at
                        FROM co_preventivi
                        JOIN dt_righe_ddt ON co_preventivi.id = dt_righe_ddt.idpreventivo
                        WHERE co_preventivi.idanagrafica ='.prepare($id_anagrafica).'
                        AND dt_righe_ddt.idarticolo ='.prepare($riga['idarticolo']).'
                        ORDER BY dt_righe_ddt.updated_at DESC'
                    )[0];
