...
foreach ($righe as $riga) {
    ...
    $id_sede = $ddt->direzione == 'uscita' ? $ddt->id_sede_destinazione : $ddt->id_sede_partenza;
...
function modificaIvaRighe(righe) {
    if (righe.length > 0) {
        openModal("'.tr('Modifica IVA').'", globals.rootdir + "/actions.php?id_module=" + globals.id_module + "&id_record=" + globals.id_record + "&op=visualizza-modifica-iva&tipo_documento=ddt&righe=" + righe.join(','));
    }
}
