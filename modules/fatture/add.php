...
$id_tipo_documento = Tipo::where('predefined', 1)->where('dir', $dir)->first()->id;
...
?>
	...
	<div class="row">
		<div class="col-md-6">
			{[ "type": "select", "label": "<?php echo tr('Tipo documento'); ?>", "name": "id_tipo_documento_add", "required": 1, "values": "query=SELECT `co_tipi_documento`.`id`, CONCAT(`co_tipi_documento`.`codice_tipo_documento_fe`, ' - ', `co_tipi_documento_lang`.`title`) AS descrizione, `co_tipi_documento`.`id_segment`, `zz_segments_lang`.`title` as name_segment FROM `co_tipi_documento` LEFT JOIN `co_tipi_documento_lang` ON (`co_tipi_documento_lang`.`id_record` = `co_tipi_documento`.`id` AND `co_tipi_documento_lang`.`id_lang` = <?php echo prepare(Models\Locale::getDefault()->id); ?>) INNER JOIN `zz_segments` ON `zz_segments`.`id` = `co_tipi_documento`.`id_segment` LEFT JOIN `zz_segments_lang` ON (`zz_segments`.`id` = `zz_segments_lang`.`id_record` AND `zz_segments_lang`.`id_lang` = <?php echo prepare(Models\Locale::getDefault()->id); ?>) WHERE `co_tipi_documento`.`enabled` = 1 AND `co_tipi_documento`.`dir` = '<?php echo $dir; ?>' ORDER BY `co_tipi_documento`.`codice_tipo_documento_fe`", "value": "<?php echo $id_tipo_documento; ?>" ]}
		</div>

		<div class="col-md-6">
			{[ "type": "select", "label": "<?php echo tr('Sezionale'); ?>", "name": "id_segment_add", "required": 1, "ajax-source": "segmenti", "select-options": <?php echo json_encode(['id_module' => $id_module, 'is_sezionale' => 1]); ?>, "value": "<?php echo Tipo::where('id', $id_tipo_documento)->where('dir', $dir)->first()->id_segment; ?>" ]}
		</div>
	</div>
...
$(document).ready(function () {
    ...
    input("id_tipo_documento_add").change(function () {
        ...
        $.ajax({
            ...
            data: {
                id_module: globals.id_module,
                id_tipo_documento:input(this).get(),
