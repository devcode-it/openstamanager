...
$righe = $dbo->fetchArray(
    'SELECT mg_articoli.descrizione, co_righe_documenti.*
            ...
            <?php foreach ($righe as $riga) { ?>
                <?php
                    ...
                    $ultimo_prezzo_preventivo = $dbo->fetchArray(
                        'SELECT co_righe_documenti.idarticolo, co_righe_documenti.prezzo_unitario, date(co_preventivi.updated_at) as updated_at
                        FROM co_preventivi
                        JOIN co_righe_documenti ON co_preventivi.id = co_righe_documenti.idpreventivo
                        WHERE co_preventivi.idanagrafica ='.prepare($id_anagrafica).'
                        AND co_righe_documenti.idarticolo ='.prepare($riga['idarticolo']).'
                        ORDER BY co_righe_documenti.updated_at DESC'
                    )[0];
                    ...
                    $ultimo_prezzo_vendita = $dbo->fetchArray(
                        ...
                        FROM co_righe_documenti WHERE idarticolo='.prepare($riga['idarticolo']).' AND
