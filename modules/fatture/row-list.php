    ...
    <tr data-id="'.$r['id'].'" '.$extra.'>';
        ...
        if ($records[0]['stato'] != 'Pagato' && $records[0]['stato'] != 'Emessa' && strpos($r['descrizione'], 'SCONTO') === false) {
            ...
            echo "
                    ...
                    <a class='btn btn-xs btn-danger' title='Rimuovi questa riga...' onclick=\"if( confirm('Rimuovere questa riga dalla fattura?') ){ $('#delete-form-".$r['id']."').submit(); }\"><i class='fa fa-trash-o'></i></a>
                </div>
            </form>";
        }
