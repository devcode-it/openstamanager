...
<?php
$elementi = $dbo->fetchArray('SELECT `in_interventi`.`id`, `in_interventi`.`codice` AS numero, `data_richiesta` AS data, `in_stati_intervento_lang`.`title` AS stato_documento, "Attività" AS tipo_documento FROM `in_interventi` INNER JOIN `my_impianti_interventi` ON `in_interventi`.`id`=`my_impianti_interventi`.`id_intervento` LEFT JOIN in_stati_intervento ON in_interventi.id_stato=in_stati_intervento.id LEFT JOIN `in_stati_intervento_lang` ON (`in_stati_intervento`.`id` = `in_stati_intervento_lang`.`id_record` AND `in_stati_intervento_lang`.`id_lang` = '.prepare(Models\Locale::getDefault()->id).') WHERE `my_impianti_interventi`.`id_impianto` = '.prepare($id_record).'
UNION
