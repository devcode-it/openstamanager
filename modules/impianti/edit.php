...
UNION
SELECT `co_contratti`.`id`, `numero` AS numero, `data_bozza` AS data, `co_staticontratti_lang`.`title` AS stato_documento, "Contratto" AS tipo_documento FROM `co_contratti` INNER JOIN `my_impianti_contratti` ON `co_contratti`.`id`=`my_impianti_contratti`.`idcontratto` LEFT JOIN co_staticontratti ON co_contratti.id_stato=co_staticontratti.id LEFT JOIN `co_staticontratti_lang` ON (`co_staticontratti`.`id` = `co_staticontratti_lang`.`id_record` AND `co_staticontratti_lang`.`id_lang` = '.prepare(Models\Locale::getDefault()->id).') WHERE `my_impianti_contratti`.`idimpianto` = '.prepare($id_record));
$class = '';
