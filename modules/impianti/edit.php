...
<?php
$elementi = $dbo->fetchArray('SELECT `in_interventi`.`id`, `in_interventi`.`codice` AS numero, `data_richiesta` AS data, `in_statiintervento_lang`.`title` AS stato_documento, "Attività" AS tipo_documento FROM `in_interventi` INNER JOIN `my_impianti_interventi` ON `in_interventi`.`id`=`my_impianti_interventi`.`idintervento` LEFT JOIN in_statiintervento ON in_interventi.id_stato=in_statiintervento.id LEFT JOIN `in_statiintervento_lang` ON (`in_statiintervento`.`id` = `in_statiintervento_lang`.`id_record` AND `in_statiintervento_lang`.`id_lang` = '.prepare(Models\Locale::getDefault()->id).') WHERE `my_impianti_interventi`.`idimpianto` = '.prepare($id_record).'
UNION
