...
UNION
SELECT `co_contratti`.`id`, `numero` AS numero, `data_bozza` AS data, `co_stati_contratti_lang`.`title` AS stato_documento, "Contratto" AS tipo_documento FROM `co_contratti` INNER JOIN `my_impianti_contratti` ON `co_contratti`.`id`=`my_impianti_contratti`.`id_contratto` LEFT JOIN co_stati_contratti ON co_contratti.id_stato=co_stati_contratti.id LEFT JOIN `co_stati_contratti_lang` ON (`co_stati_contratti`.`id` = `co_stati_contratti_lang`.`id_record` AND `co_stati_contratti_lang`.`id_lang` = '.prepare(Models\Locale::getDefault()->id).') WHERE `my_impianti_contratti`.`id_impianto` = '.prepare($id_record));
$class = '';
