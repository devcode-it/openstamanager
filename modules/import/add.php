...
?><form action="" method="post" id="add-form" enctype="multipart/form-data">
	...
	<div class="row">
		...
		<div class="col-md-6">
			{[ "type": "select", "label": "<?php echo tr('Modulo'); ?>", "name": "id_import", "required": 1, "values": "query=SELECT `zz_imports`.`id`, `zz_imports_lang`.`title` AS text FROM `zz_imports` LEFT JOIN `zz_imports_lang` ON (`zz_imports`.`id`=`zz_imports_lang`.`id_record` AND `zz_imports_lang`.`id_lang` = <?php echo prepare(Models\Locale::getDefault()->id); ?>) ORDER BY `title`" ]}
		</div>
	</div>
