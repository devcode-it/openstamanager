...
<div class="row">
    <div class="col-md-12">
        {[ "type": "select", "label": "'.tr('Ordine').'", "name": "id_documento", "values": "query=SELECT `or_ordini`.`id`, CONCAT(IF(`numero_esterno` != \'\', `numero_esterno`, `numero`), \' del \', DATE_FORMAT(`data`, \'%d-%m-%Y\')) AS descrizione FROM `or_ordini` INNER JOIN `or_statiordine` ON `or_ordini`.`id_stato` = `or_statiordine`.`id` LEFT JOIN `or_statiordine_lang` ON (`or_statiordine`.`id` = `or_statiordine_lang`.`id_record` AND `or_statiordine_lang`.`id_lang` = '.prepare(Models\Locale::getDefault()->id).') INNER JOIN `or_tipiordine` ON `or_ordini`.`idtipoordine` = `or_tipiordine`.`id` INNER JOIN `or_righe_ordini` ON `or_ordini`.`id` = `or_righe_ordini`.`id_ordine` WHERE `or_statiordine`.`is_fatturabile` = 1 AND (`or_righe_ordini`.`qta` - `or_righe_ordini`.`qta_evasa`) > 0 AND ((`or_tipiordine`.`dir` = \"entrata\" AND `id_anagrafica`='.prepare($id_anagrafica).') OR `or_tipiordine`.`dir` = \"uscita\") GROUP BY or_ordini.id ORDER BY `data` DESC, `numero` DESC " ]}
    </div>
</div>
