...
if ($user['gruppo'] == 'Tecnici') {
    $show_costi = !empty($user['id_anagrafica']) && setting('Mostra i prezzi al tecnico');
}

// Stato dell'intervento
$rss = $dbo->fetchArray('SELECT `is_bloccato` AS flag_completato FROM `in_stati_intervento` INNER JOIN `in_interventi` ON `in_stati_intervento`.`id` = `in_interventi`.`id_stato` WHERE `in_interventi`.`id`='.prepare($id_record));
...
<div class="row">
    ...
    <div class="offset-md-4 col-md-4">
        {[ "type": "select", "label": "'.tr('Tecnico').'", "name": "nuovo_tecnico", "placeholder": "'.tr('Seleziona un tecnico').'", "ajax-source": "tecnici", "icon-after": "add|'.Module::where('name', 'Anagrafiche')->first()->id.'|tipoanagrafica=Tecnico&readonly_tipo=1" ]}
    </div>
