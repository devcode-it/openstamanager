...
switch (post('op')) {
    ...
    case 'create_invoice':
        ...
        $where = '';
        $query = 'SELECT `in_interventi`.*, IFNULL(`sessioni_min`.`min_orario_inizio`, `in_interventi`.`data_richiesta`) AS data, `in_stati_intervento_lang`.`title` AS stato, `in_interventi`.`codice` AS codice_intervento FROM `in_interventi` INNER JOIN `in_stati_intervento` ON `in_interventi`.`id_stato`=`in_stati_intervento`.`id` LEFT JOIN `in_stati_intervento_lang` ON (`in_stati_intervento_lang`.`id_record`=`in_stati_intervento`.`id` AND `in_stati_intervento_lang`.`id_lang`='.prepare(Models\Locale::getDefault()->id).') LEFT JOIN (SELECT `id_intervento`, MIN(`orario_inizio`) AS min_orario_inizio FROM `in_interventi_tecnici` GROUP BY `id_intervento`) AS `sessioni_min` ON `sessioni_min`.`id_intervento` = `in_interventi`.`id` WHERE `in_stati_intervento`.`is_fatturabile`=1 AND `in_interventi`.`id` NOT IN (SELECT `id_intervento` FROM `co_righe_documenti` WHERE `id_intervento` IS NOT NULL) AND `in_interventi`.`id` IN ('.implode(',', $id_records).')';
