...
<?php
                if (setting('Cambia automaticamente stato attività fatturate')) {
                    ...
                    if ($intervento->stato->id == $id_stato_fatt) {
                        echo '
                        {[ "type": "select", "label": "'.tr('Stato').'", "name": "id_stato", "required": 1, "values": "query=SELECT `in_stati_intervento`.`id`, `title` as descrizione, `colore` AS _bgcolor_ FROM `in_stati_intervento`  LEFT JOIN `in_stati_intervento_lang` ON (`in_stati_intervento`.`id` = `in_stati_intervento_lang`.`id_record` AND `in_stati_intervento_lang`.`id_lang` ='.prepare(Locale::getDefault()->id).') WHERE `deleted_at` IS NULL ORDER BY `title`", "value": "$id_stato$", "class": "unblockable" ]}';
                    } else {
                        echo '
                        {[ "type": "select", "label": "'.tr('Stato').'", "name": "id_stato", "required": 1, "values": "query=SELECT `in_stati_intervento`.`id`, `title` as descrizione, `colore` AS _bgcolor_ FROM `in_stati_intervento`  LEFT JOIN `in_stati_intervento_lang` ON (`in_stati_intervento`.`id` = `in_stati_intervento_lang`.`id_record` AND `in_stati_intervento_lang`.`id_lang` ='.prepare(Locale::getDefault()->id).') WHERE `in_stati_intervento`.`id`!='.prepare($id_stato_fatt).' AND `deleted_at` IS NULL ORDER BY `title`", "value": "$id_stato$", "class": "unblockable" ]}';
                    }
                } else {
                    echo '
                    {[ "type": "select", "label": "'.tr('Stato').'", "name": "id_stato", "required": 1, "values": "query=SELECT `in_stati_intervento`.`id`, `title` as descrizione, `colore` AS _bgcolor_ FROM `in_stati_intervento`  LEFT JOIN `in_stati_intervento_lang` ON (`in_stati_intervento`.`id` = `in_stati_intervento_lang`.`id_record` AND `in_stati_intervento_lang`.`id_lang` ='.prepare(Locale::getDefault()->id).') WHERE `deleted_at` IS NULL ORDER BY `title`", "value": "$id_stato$", "class": "unblockable" ]}';
...
if (!$block_edit) {
    ...
    $ddt_query = 'SELECT
        ...
        FROM
            `dt_ddt`
            LEFT JOIN `dt_causale_t` ON `dt_causale_t`.`id` = `dt_ddt`.`id_causale_t`
        ...
        WHERE
            ...
            AND `dt_stati_ddt_lang`.`title` IN ("Evaso", "Parzialmente evaso", "Parzialmente fatturato")
            AND `dt_tipi_ddt`.`dir` = '.prepare($intervento->direzione).'
    ...
    $ordini_query = 'SELECT
        ...
        WHERE
            ((`or_tipi_ordine`.`dir` = "entrata" AND `id_anagrafica`='.prepare($record['id_anagrafica']).') || `or_tipi_ordine`.`dir` = "uscita")
