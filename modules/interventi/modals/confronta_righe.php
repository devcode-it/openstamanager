...
$righe = $dbo->fetchArray(
    ...
    FROM 
        `in_righe_interventi`
        INNER JOIN `mg_articoli` ON `mg_articoli`.`id` = `in_righe_interventi`.`id_articolo`
        ...
        <tbody>
            <?php foreach ($righe as $riga) { ?>
                ...
                $ultimo_prezzo_preventivo = $dbo->fetchArray(
                    'SELECT
                        `in_righe_interventi`.`id_articolo`,
                    ...
                    FROM
                        ...
                        INNER JOIN `in_righe_interventi` ON `in_righe_interventi`.`id_articolo` = `mg_articoli`.`id`
                    ...
                    WHERE
                        `co_preventivi`.`id_anagrafica` ='.prepare($id_anagrafica).' AND `in_righe_interventi`.`id_articolo` ='.prepare($riga['id_articolo']).' AND `co_stati_preventivi_lang`.`title` NOT IN ("Bozza", "In attesa di conferma", "Rifiutato")
                    GROUP BY 
                ...
                $ultimo_prezzo_vendita = $dbo->fetchArray(
                    ...
                    FROM
                        ...
                        LEFT JOIN `co_stati_documento_lang` ON (`co_stati_documento_lang`.`id_stato` = `co_stati_documento`.`id` AND `co_stati_documento_lang`.`id_lang` = '.prepare(Models\Locale::getDefault()->id).')
                    WHERE
