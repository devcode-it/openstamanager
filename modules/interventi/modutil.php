...
if (!function_exists('add_tecnico')) {
    function add_tecnico($id_intervento, $id_tecnico, $inizio, $fine, $id_contratto = null)
    {
        ...
        $righe_aggiuntive = database()->fetchArray('SELECT * FROM in_righe_tipi_interventi WHERE id_tipo_intervento='.prepare($sessione->id_tipo_intervento));
...
if (!function_exists('aggiungi_intervento_in_fattura')) {
    function aggiungi_intervento_in_fattura($id_intervento, $id_fattura, $descrizione, $id_iva, $id_conto, $id_rivalsa_inps = false, $id_ritenuta_acconto = false, $calcolo_ritenuta_acconto = false)
    {
        ...
        if (setting('Cambia automaticamente stato attività fatturate')) {
            $dbo->query("UPDATE `in_interventi` SET `id_stato`=(SELECT `id` FROM `in_stati_intervento` WHERE `codice`='FAT') WHERE `id`=".prepare($id_intervento));
        }
    }
}
