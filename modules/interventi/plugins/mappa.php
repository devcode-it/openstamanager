...
if (get('op') == 'getmappa') {
    ...
    for ($i = 0; $i < sizeof($rs1); ++$i) {// elenco delle righe
        ...
        if ($rs[0]['lat'] && $rs[0]['lng']) {
            ...
            $altri_interventi = $dbo->fetchArray('SELECT * FROM in_interventi WHERE idsede_destinazione='.prepare($id_sede).' AND id_anagrafica='.prepare($val).' AND id IN ('.implode(',', array_map(prepare(...), array_column($rs1, 'id'))).')');
