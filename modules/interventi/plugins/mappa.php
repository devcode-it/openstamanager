...
if (get('op') == 'getmappa') {
    ...
    for ($i = 0; $i < sizeof($rs1); ++$i) {// elenco delle righe
        $val = html_entity_decode((string) $rs1[$i]['id_anagrafica']);
        $id_sede = $dbo->selectOne('in_interventi', '*', ['id' => $rs1[$i]['id']])['id_sede_destinazione'];
        if ($id_sede) {
            $query = 'SELECT *, nome_sede AS ragione_sociale FROM an_sedi WHERE id='.prepare($id_sede);
            $rs = $dbo->fetchArray($query);
        } else {
            $query = 'SELECT *, ragione_sociale FROM an_anagrafiche WHERE id='.prepare($val);
            $rs = $dbo->fetchArray($query);
        }
        ...
        if ($rs[0]['lat'] && $rs[0]['lng']) {
            ...
            $altri_interventi = $dbo->fetchArray('SELECT * FROM in_interventi WHERE id_sede_destinazione='.prepare($id_sede).' AND id_anagrafica='.prepare($val).' AND id IN ('.implode(',', array_map(prepare(...), array_column($rs1, 'id'))).')');
