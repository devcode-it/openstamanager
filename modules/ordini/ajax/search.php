...
$query .= ' OR `or_ordini`.`id` IN (SELECT DISTINCT `or_righe_ordini`.`id_ordine` FROM `or_righe_ordini` LEFT JOIN `mg_articoli` ON `or_righe_ordini`.`id_articolo` = `mg_articoli`.`id` LEFT JOIN `mg_articoli_lang` ON (`mg_articoli`.`id` = `mg_articoli_lang`.`id_record` AND `mg_articoli_lang`.`id_lang` = '.prepare(Models\Locale::getDefault()->id).') WHERE `mg_articoli`.`codice` LIKE '.prepare('%'.$term.'%').' OR `mg_articoli_lang`.`title` LIKE '.prepare('%'.$term.'%').')';
