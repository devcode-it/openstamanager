...
$righe = $dbo->fetchArray(
    'SELECT mg_articoli.descrizione, or_righe_ordini.*
            ...
            <?php foreach ($righe as $riga) { ?>
                <?php
                    ...
                    $ultimo_prezzo_preventivo = $dbo->fetchArray(
                        'SELECT or_righe_ordini.idarticolo, or_righe_ordini.prezzo_unitario, date(co_preventivi.updated_at) as updated_at
                        FROM co_preventivi
                        JOIN or_righe_ordini ON co_preventivi.id = or_righe_ordini.idpreventivo
                        WHERE co_preventivi.idanagrafica ='.prepare($id_anagrafica).'
                        AND or_righe_ordini.idarticolo ='.prepare($riga['idarticolo']).'
                        ORDER BY or_righe_ordini.updated_at DESC'
                    )[0];
