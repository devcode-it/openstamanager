...
switch ($resource) {
    ...
    case 'pagamenti':
        ...
        FROM `co_pagamenti`
            ...
            LEFT JOIN `co_banche` banca_cliente ON `banca_cliente`.`id_anagrafica` = '.prepare($superselect['id_anagrafica']).' AND `banca_cliente`.`deleted_at` IS NULL
            LEFT JOIN `co_banche` banca_vendite ON `co_pagamenti`.`id_conto_vendite` = `banca_vendite`.`id_piano_dei_conti3` AND `banca_vendite`.`id_anagrafica` = '.prepare($id_azienda).' AND `banca_vendite`.`deleted_at` IS NULL
            LEFT JOIN `co_banche` banca_acquisti ON `co_pagamenti`.`id_conto_acquisti` = `banca_acquisti`.`id_piano_dei_conti3` AND `banca_acquisti`.`id_anagrafica` = '.prepare($id_azienda).' AND `banca_acquisti`.`deleted_at` IS NULL
        |where|
        GROUP BY
