...
foreach ($results as $result) {
    ...
    $elementi = $dbo->fetchArray('SELECT `co_documenti`.`id`, `co_documenti`.`data`, `co_documenti`.`numero`, `co_documenti`.`numero_esterno`, `co_tipi_documento_lang`.`title` AS tipo_documento, `co_tipi_documento`.`dir`, NULL AS `deleted_at` FROM `co_documenti` INNER JOIN `co_tipi_documento` ON `co_tipi_documento`.`id` = `co_documenti`.`id_tipo_documento` LEFT JOIN `co_tipi_documento_lang` ON (`co_tipi_documento`.`id`=`co_tipi_documento_lang`.`id_record` AND `co_tipi_documento_lang`.`id_lang`= '.prepare(Models\Locale::getDefault()->id).') WHERE `co_documenti`.`id_pagamento` = '.prepare($id_record).'

UNION
SELECT `or_ordini`.`id`, `or_ordini`.`data`, `or_ordini`.`numero`, `or_ordini`.`numero_esterno`, `or_tipi_ordine_lang`.`title` AS tipo_documento, `or_tipi_ordine`.`dir`, NULL AS `deleted_at` FROM `or_ordini` INNER JOIN `or_tipi_ordine` ON `or_tipi_ordine`.`id` = `or_ordini`.`id_tipo_ordine` LEFT JOIN `or_tipi_ordine_lang` ON (`or_tipi_ordine`.`id`=`or_tipi_ordine_lang`.`id_record` AND `or_tipi_ordine_lang`.`id_lang`= '.prepare(Models\Locale::getDefault()->id).') WHERE `or_ordini`.`id_pagamento` = '.prepare($id_record).'

UNION
SELECT `dt_ddt`.`id`, `dt_ddt`.`data`, `dt_ddt`.`numero`, `dt_ddt`.`numero_esterno`, `dt_tipi_ddt_lang`.`title` AS tipo_documento, `dt_tipi_ddt`.`dir`, NULL AS `deleted_at` FROM `dt_ddt` INNER JOIN `dt_tipi_ddt` ON `dt_tipi_ddt`.`id` = `dt_ddt`.`id_tipo_ddt` LEFT JOIN `dt_tipi_ddt_lang` ON (`dt_tipi_ddt`.`id`=`dt_tipi_ddt_lang`.`id_record` AND `dt_tipi_ddt_lang`.`id_lang`= '.prepare(Models\Locale::getDefault()->id).') WHERE `dt_ddt`.`id_pagamento` = '.prepare($id_record).'

UNION
SELECT `co_contratti`.`id`, `co_contratti`.`data_bozza`, `co_contratti`.`numero`, 0 AS numero_esterno , "Contratto" AS tipo_documento, 0 AS dir, NULL AS `deleted_at` FROM `co_contratti` WHERE `co_contratti`.`id_pagamento` = '.prepare($id_record).'

UNION
SELECT `co_preventivi`.`id`, `co_preventivi`.`data_bozza`, `co_preventivi`.`numero`, 0 AS numero_esterno , "Preventivo" AS tipo_documento, 0 AS dir, NULL AS `deleted_at` FROM `co_preventivi` WHERE `co_preventivi`.`id_pagamento` = '.prepare($id_record).'
