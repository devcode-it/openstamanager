    ...
    case 'update':
        ...
        if (isset($descrizione)) {
            if ($dbo->fetchNum('SELECT * FROM `dt_porto` WHERE `descrizione`='.prepare($descrizione).' AND `id`!='.prepare($id_record)) == 0) {
                ...
                $_SESSION['errors'][] = str_replace('_TYPE_', 'porto', _("E' già presente una tipologia di _TYPE_ con la stessa descrizione!"));
            }
        } else {
    ...
    case 'add':
        ...
        if (isset($descrizione)) {
            if ($dbo->fetchNum('SELECT * FROM `dt_porto` WHERE `descrizione`='.prepare($descrizione)) == 0) {
                ...
                $_SESSION['infos'][] = str_replace('_TYPE_', 'porto', _('Aggiunta nuova tipologia di _TYPE_'));
            } else {
                $_SESSION['errors'][] = str_replace('_TYPE_', 'porto', _("E' già presente una tipologia di _TYPE_ con la stessa descrizione!"));
            }
        } else {
...
UNION SELECT id FROM co_preventivi WHERE idporto='.prepare($id_record));
        ...
        if (isset($id_record) && !empty($documenti)) {
            ...
            $_SESSION['infos'][] = str_replace('_TYPE_', 'porto', _('Tipologia di _TYPE_ eliminata con successo!'));
        }else{
            $_SESSION['errors'][] = _('Sono presenti dei documenti collegati a questo porto!');
        }
