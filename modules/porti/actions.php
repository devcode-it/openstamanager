...
switch (filter('op')) {
    case 'update':
        ...
        if ($dbo->fetchNum('SELECT * FROM `dt_porto` LEFT JOIN `dt_porto_lang` ON (`dt_porto_lang`.`id_record` = `dt_porto`.`id` AND `dt_porto_lang`.`id_lang` = '.prepare(\App::getLang()).') WHERE `name`='.prepare($descrizione).' AND `dt_porto`.`id`!='.prepare($id_record)) == 0) {
            if (!empty($predefined)) {
    ...
    case 'add':
        ...
        if ($dbo->fetchNum('SELECT * FROM `dt_porto` LEFT JOIN `dt_porto_lang` 0N (`dt_porto_lang`.`id_record` = `dt_porto`.`id` AND `dt_porto_lang`.`id_lang` = '.prepare(\App::getLang()).') WHERE `name`='.prepare($descrizione)) == 0) {
            $dbo->insert('dt_porto', [
                'created_at' => 'NOW()',
            ]);
