...
$stati = get('pianificabile') ? 'SELECT `co_stati_preventivi`.`id`, `co_stati_preventivi_lang`.`title` as descrizione FROM `co_stati_preventivi`  LEFT JOIN `co_stati_preventivi_lang` ON (`co_stati_preventivi`.`id` = `co_stati_preventivi_lang`.`id_record` AND `co_stati_preventivi_lang`.`id_lang` = '.prepare(Models\Locale::getDefault()->id).') WHERE `is_pianificabile`=1 AND `co_stati_preventivi`.`deleted_at` is NULL ORDER BY `title`' : 'SELECT `co_stati_preventivi`.`id`, `co_stati_preventivi_lang`.`title` as descrizione FROM `co_stati_preventivi` LEFT JOIN `co_stati_preventivi_lang` ON (`co_stati_preventivi`.`id` = `co_stati_preventivi_lang`.`id_record` AND `co_stati_preventivi_lang`.`id_lang` = '.prepare(Models\Locale::getDefault()->id).') WHERE `co_stati_preventivi`.`deleted_at` is NULL ORDER BY `title`';
...
?><form action="" method="post" id="add-form">
	...
	<div class="row">
		...
		<div class="col-md-4">
				{[ "type": "select", "label": "<?php echo tr('Sede destinazione'); ?>", "name": "id_sede_destinazione", "ajax-source": "sedi", "select-options": <?php echo json_encode(['id_anagrafica' => $id_anagrafica]); ?>, "placeholder": "Sede legale" ]}
		</div>
	...
	<div class="row">
		...
		<div class="col-md-6">
            {[ "type": "select", "label": "<?php echo tr('Stato'); ?>", "name": "id_stato", "required": 1, "value": "<?php echo $stato; ?>", "values": "query=<?php echo $stati; ?>" ]}
        </div>
	</div>
...
<script>
	$('#modals > div #id_anagrafica').change(function() {
		$(".modal #id_sede_destinazione").selectReset();
