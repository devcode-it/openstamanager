...
// Sede
if ($preventivo->idsede_destinazione) {
    $sede = $dbo->selectOne('an_sedi', '*', ['id' => $preventivo->idsede_destinazione]);
} else {
...
// Referente
$referente = null;
if ($preventivo->idreferente) {
    $referente = $dbo->selectOne('an_referenti', '*', ['id' => $preventivo->idreferente]);
}
...
$perc_ore = 0;
$color = 'danger';
if ($preventivo->id_contratto) {
    $contratto = Contratto::find($preventivo->id_contratto);
...
// Ordine
$ordine = null;
if ($preventivo->id_ordine) {
    $ordine = Ordine::find($preventivo->id_ordine);
}
...
// Insoluti
$insoluti = Scadenza::where('idanagrafica', $preventivo->idanagrafica)
    ->whereRaw('co_scadenziario.da_pagare > co_scadenziario.pagato')
    ->whereRaw('co_scadenziario.scadenza < NOW()')
    ->count();

// Logo
$logo = Upload::where('id_module', (new Module())->getByField('title', 'Anagrafiche'))->where('id_record', $preventivo->idanagrafica)->where('name', 'Logo azienda')->first()->filename;
    ...
    <div class="col-md-4">
        <div class="card card-info">
            <div class="card-header">
                <h3 class="card-title"><i class="fa fa-map"></i> '.tr('Panoramica preventivo num. ').$preventivo->numero.'</h3>
            </div>
            <div class="card-body">
...
// Geolocalizzazione
$anagrafica_cliente = $preventivo->anagrafica;
$sede_cliente = $anagrafica_cliente->sedeLegale;
if (!empty($preventivo->idsede_destinazione)) {
    $sede_cliente = Sede::find($preventivo->idsede_destinazione);
}
