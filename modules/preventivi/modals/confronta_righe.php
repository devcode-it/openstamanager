...
$righe = $dbo->fetchArray(
    'SELECT mg_articoli.descrizione, co_righe_preventivi.*
            ...
            <?php foreach ($righe as $riga) { ?>
                <?php
                    ...
                    $ultimo_prezzo_preventivo = $dbo->fetchArray(
                        'SELECT co_righe_preventivi.idarticolo, co_righe_preventivi.prezzo_unitario, date(co_preventivi.updated_at) as updated_at
                        FROM co_preventivi
                        JOIN co_righe_preventivi ON co_preventivi.id = co_righe_preventivi.idpreventivo
                        WHERE co_preventivi.idanagrafica ='.prepare($id_anagrafica).'
                        AND co_righe_preventivi.idarticolo ='.prepare($riga['idarticolo']).'
                        ORDER BY co_righe_preventivi.updated_at DESC'
                    )[0];
