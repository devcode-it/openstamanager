        ...
        <tbody>
            <?php foreach ($righe as $riga) { ?>
                ...
                $ultimo_prezzo_preventivo = $dbo->fetchArray(
                    'SELECT
                        `co_righe_preventivi`.`id_articolo`,
                    ...
                    WHERE
                        `co_preventivi`.`id_anagrafica` ='.prepare($id_anagrafica).' AND `co_righe_preventivi`.`id_articolo` ='.prepare($riga['id_articolo']).' AND `co_stati_preventivi_lang`.`title` NOT IN ("Bozza", "In attesa di conferma", "Rifiutato")
                    GROUP BY 
                ...
                $ultimo_prezzo_vendita = $dbo->fetchArray(
                    ...
                    FROM
                        ...
                        INNER JOIN `co_righe_preventivi` ON `co_righe_preventivi`.`id_articolo` = `mg_articoli`.`id`
