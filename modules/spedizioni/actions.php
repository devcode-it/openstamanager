...
switch (filter('op')) {
    ...
    case 'delete':
        $documenti = $dbo->fetchNum('SELECT `id` FROM `dt_ddt` WHERE `id_spedizione`='.prepare($id_record).'
            UNION SELECT `id` FROM `co_documenti` WHERE `id_spedizione`='.prepare($id_record));
