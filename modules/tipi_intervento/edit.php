...
$id_tipi = $dbo->select('in_tipi_intervento_tipologie', '*', [], ['id_tipo_intervento' => $id_record]);
...
$results = $dbo->fetchArray('SELECT GROUP_CONCAT(DISTINCT `id_gruppo` SEPARATOR \',\') AS gruppi FROM `in_tipi_intervento_groups` WHERE `id_tipo_intervento`='.prepare($id_record));
