...
$utenti = $dbo->fetchArray('
    SELECT
        ...
        GROUP_CONCAT(`an_tipi_anagrafiche_lang`.`title` SEPARATOR ", ") AS tipo
    FROM
        `zz_users`
        LEFT JOIN `an_anagrafiche` ON `an_anagrafiche`.`id` = `zz_users`.`id_anagrafica`
        LEFT JOIN `an_tipi_anagrafiche_anagrafiche` ON `an_tipi_anagrafiche_anagrafiche`.`id_anagrafica` = `zz_users`.`id_anagrafica`
...
if (!empty($utenti)) {
    ...
    foreach ($utenti as $utente) {
        ...
        $sedi = $dbo->fetchOne('SELECT GROUP_CONCAT(nome_sede SEPARATOR ", "  ) as nome_sede FROM zz_user_sedi INNER JOIN ((SELECT "0" AS id, "Sede legale" AS nome_sede) UNION (SELECT id, nome_sede FROM an_sedi)) sedi ON zz_user_sedi.id_sede=sedi.id WHERE id_user='.prepare($utente['id']).' GROUP BY id_user')['nome_sede'];
