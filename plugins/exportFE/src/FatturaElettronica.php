...
class FatturaElettronica implements \Stringable
{
    ...
    public function getContratti()
    {
        if (empty($this->contratti)) {
            ...
            $contratti = $database->fetchArray('SELECT DISTINCT `id_documento_fe` AS id_documento, `num_item`, `codice_cig`, `codice_cup` FROM `co_contratti` INNER JOIN `co_righe_documenti` ON `co_righe_documenti`.`id_contratto` = `co_contratti`.`id` WHERE `co_righe_documenti`.`id_documento` = '.prepare($documento['id']).' AND `id_documento_fe` IS NOT NULL AND `co_righe_documenti`.`id_preventivo` = 0 AND `co_righe_documenti`.`id_intervento` IS NULL AND `co_righe_documenti`.`id_ordine` = 0 AND `co_righe_documenti`.`id_ddt` = 0');

            $preventivi = $database->fetchArray('SELECT DISTINCT `id_documento_fe` AS id_documento, `num_item`, `codice_cig`, `codice_cup` FROM `co_preventivi` INNER JOIN `co_righe_documenti` ON `co_righe_documenti`.`id_preventivo` = `co_preventivi`.`id` WHERE `co_righe_documenti`.`id_documento` = '.prepare($documento['id']).' AND `id_documento_fe` IS NOT NULL AND `co_righe_documenti`.`id_contratto` = 0 AND `co_righe_documenti`.`id_intervento` IS NULL AND `co_righe_documenti`.`id_ordine` = 0 AND `co_righe_documenti`.`id_ddt` = 0');

            $interventi = $database->fetchArray('SELECT DISTINCT `id_documento_fe` AS id_documento, `num_item`, `codice_cig`, `codice_cup` FROM `in_interventi` INNER JOIN `co_righe_documenti` ON `co_righe_documenti`.`id_intervento` = `in_interventi`.`id` WHERE `co_righe_documenti`.`id_documento` = '.prepare($documento['id']).' AND `id_documento_fe` IS NOT NULL AND `co_righe_documenti`.`id_contratto` = 0 AND `co_righe_documenti`.`id_preventivo` = 0 AND `co_righe_documenti`.`id_ordine` = 0 AND `co_righe_documenti`.`id_ddt` = 0');

            $ddt = $database->fetchArray('SELECT DISTINCT `id_documento_fe` AS id_documento, `num_item`, `codice_cig`, `codice_cup` FROM `dt_ddt` INNER JOIN `co_righe_documenti` ON `co_righe_documenti`.`id_ddt` = `dt_ddt`.`id` WHERE `co_righe_documenti`.`id_documento` = '.prepare($documento['id']).' AND `id_documento_fe` IS NOT NULL AND `co_righe_documenti`.`id_contratto` = 0 AND `co_righe_documenti`.`id_preventivo` = 0 AND `co_righe_documenti`.`id_intervento` IS NULL AND `co_righe_documenti`.`id_ordine` = 0');
    ...
    public function getDDT()
    {
        if (empty($this->ddt)) {
            ...
            $ddt = $database->fetchArray('SELECT `dt_ddt`.`numero_esterno` AS id_documento, `co_righe_documenti`.`order` AS riferimento_linea, `dt_ddt`.`data` FROM `dt_ddt` INNER JOIN `co_righe_documenti` ON `co_righe_documenti`.`id_ddt` = `dt_ddt`.`id` WHERE `co_righe_documenti`.`id_documento` = '.prepare($documento['id']));
    ...
    protected static function getDatiTrasporto($fattura)
    {
        ...
        $causale = $database->fetchOne('SELECT `title` FROM `dt_causale_t` LEFT JOIN `dt_causale_t_lang` ON (`dt_causale_t`.`id`=`dt_causale_t_lang`.`id_record` AND `dt_causale_t_lang`.`id_lang`='.prepare(\Models\Locale::getDefault()->id).') WHERE `dt_causale_t`.`id` = '.prepare($documento['id_causale_t']))['name'];
        $aspetto = $database->fetchOne('SELECT `title` FROM `dt_aspetto_beni` LEFT JOIN `dt_aspetto_beni_lang` ON (`dt_aspetto_beni`.`id`=`dt_aspetto_beni_lang`.`id_record` AND `dt_aspetto_beni_lang`.`id_lang`='.prepare(\Models\Locale::getDefault()->id).') WHERE `dt_aspetto_beni`.`id` = '.prepare($documento['id_aspetto_beni']))['name'];
    ...
    protected static function getDatiOrdineAcquisto($fattura, $lista = null)
    {
        ...
        foreach ($lista as $element) {
            ...
            } elseif (!empty($element['riferimento_linea'][0])) {
