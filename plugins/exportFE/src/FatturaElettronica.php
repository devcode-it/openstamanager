...
class FatturaElettronica implements \Stringable
{
    ...
    public function getContratti()
    {
        if (empty($this->contratti)) {
            ...
            $contratti = $database->fetchArray('SELECT DISTINCT `id_documento_fe` AS id_documento, `num_item`, `codice_cig`, `codice_cup` FROM `co_contratti` INNER JOIN `co_righe_documenti` ON `co_righe_documenti`.`id_contratto` = `co_contratti`.`id` WHERE `co_righe_documenti`.`id_documento` = '.prepare($documento['id']).' AND `id_documento_fe` IS NOT NULL AND `co_righe_documenti`.`idpreventivo` = 0 AND `co_righe_documenti`.`id_intervento` IS NULL AND `co_righe_documenti`.`id_ordine` = 0 AND `co_righe_documenti`.`id_ddt` = 0');

            $preventivi = $database->fetchArray('SELECT DISTINCT `id_documento_fe` AS id_documento, `num_item`, `codice_cig`, `codice_cup` FROM `co_preventivi` INNER JOIN `co_righe_documenti` ON `co_righe_documenti`.`idpreventivo` = `co_preventivi`.`id` WHERE `co_righe_documenti`.`id_documento` = '.prepare($documento['id']).' AND `id_documento_fe` IS NOT NULL AND `co_righe_documenti`.`id_contratto` = 0 AND `co_righe_documenti`.`id_intervento` IS NULL AND `co_righe_documenti`.`id_ordine` = 0 AND `co_righe_documenti`.`id_ddt` = 0');

            $interventi = $database->fetchArray('SELECT DISTINCT `id_documento_fe` AS id_documento, `num_item`, `codice_cig`, `codice_cup` FROM `in_interventi` INNER JOIN `co_righe_documenti` ON `co_righe_documenti`.`id_intervento` = `in_interventi`.`id` WHERE `co_righe_documenti`.`id_documento` = '.prepare($documento['id']).' AND `id_documento_fe` IS NOT NULL AND `co_righe_documenti`.`id_contratto` = 0 AND `co_righe_documenti`.`idpreventivo` = 0 AND `co_righe_documenti`.`id_ordine` = 0 AND `co_righe_documenti`.`id_ddt` = 0');

            $ddt = $database->fetchArray('SELECT DISTINCT `id_documento_fe` AS id_documento, `num_item`, `codice_cig`, `codice_cup` FROM `dt_ddt` INNER JOIN `co_righe_documenti` ON `co_righe_documenti`.`id_ddt` = `dt_ddt`.`id` WHERE `co_righe_documenti`.`id_documento` = '.prepare($documento['id']).' AND `id_documento_fe` IS NOT NULL AND `co_righe_documenti`.`id_contratto` = 0 AND `co_righe_documenti`.`idpreventivo` = 0 AND `co_righe_documenti`.`id_intervento` IS NULL AND `co_righe_documenti`.`id_ordine` = 0');
