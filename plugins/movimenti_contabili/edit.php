...
if (empty($_GET['visualizza_movimenti'])) {
    ...
    if ($modulo == 'Anagrafiche') {
        $movimenti = $dbo->fetchArray('SELECT co_movimenti.*, totale, co_piano_dei_conti3.descrizione, co_piano_dei_conti3.numero AS conto3, co_piano_dei_conti2.numero AS conto2 FROM co_movimenti LEFT JOIN co_piano_dei_conti3 ON co_movimenti.id_conto=co_piano_dei_conti3.id LEFT JOIN co_piano_dei_conti2 ON co_piano_dei_conti3.id_piano_dei_conti2=co_piano_dei_conti2.id WHERE id_anagrafica='.prepare($id_record).' GROUP BY co_movimenti.id ORDER BY data, id_mastrino');
    } else {
        $movimenti = $dbo->fetchArray('SELECT co_movimenti.*, totale, co_piano_dei_conti3.descrizione, co_piano_dei_conti3.numero AS conto3, co_piano_dei_conti2.numero AS conto2 FROM co_movimenti LEFT JOIN co_piano_dei_conti3 ON co_movimenti.id_conto=co_piano_dei_conti3.id LEFT JOIN co_piano_dei_conti2 ON co_piano_dei_conti3.id_piano_dei_conti2=co_piano_dei_conti2.id WHERE id_documento='.prepare($id_record).' GROUP BY co_movimenti.id ORDER BY data, id_mastrino');
    }
    ...
    if (!empty($movimenti)) {
        ...
        if ($modulo != 'Anagrafiche') {
            $altri_movimenti = $dbo->fetchArray('SELECT co_movimenti.*, SUM(totale) AS totale, co_piano_dei_conti3.descrizione, co_piano_dei_conti3.numero AS conto3, co_piano_dei_conti2.numero AS conto2 FROM co_movimenti LEFT JOIN co_piano_dei_conti3 ON co_movimenti.id_conto=co_piano_dei_conti3.id LEFT JOIN co_piano_dei_conti2 ON co_piano_dei_conti3.id_piano_dei_conti2=co_piano_dei_conti2.id WHERE id_documento=0 AND id_mastrino IN('.implode(',', array_map(prepare(...), $idmastrini_processati)).') GROUP BY id_mastrino, id_conto ORDER BY data, id_mastrino');
