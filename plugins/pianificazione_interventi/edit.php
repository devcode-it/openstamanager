...
<table class="table table-sm table-striped table-hover">
    ...
    foreach ($elenco_promemoria as $promemoria) {
        ...
        if ($promemoria['id_sede'] == '-1') {
            ...
            $info_sede = $dbo->fetchOne("SELECT id, CONCAT( CONCAT_WS( ' (', CONCAT_WS(', ', nome_sede, citta), indirizzo ), ')') AS descrizione FROM an_sedi WHERE id=".prepare($promemoria->id_sede))['descrizione'];
        }
        ...
        echo '
            <tr>
                ...
                <button type="button" '.$disabled.' class="btn btn-primary btn-sm '.$disabled.' " title="Pianifica intervento ora..." data-widget="tooltip" onclick="launch_modal(\'Pianifica intervento\', \''.base_path_osm().'/add.php?id_module='.Module::where('name', 'Interventi')->first()->id.'&ref=interventi_contratti&id_contratto='.$id_record.'&id_contratto_riga='.$promemoria['id'].'\');"'.(!empty($is_pianificabile) ? '' : ' disabled').'>
