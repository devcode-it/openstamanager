...
$record = $dbo->fetchOne('SELECT *, `in_tipi_intervento_lang`.`title` AS tipointervento, `in_tipi_intervento`.`tempo_standard` FROM `co_promemoria` INNER JOIN `in_tipi_intervento` ON `in_tipi_intervento`.`id` = `co_promemoria`.`id_tipo_intervento` LEFT JOIN `in_tipi_intervento_lang` ON (`in_tipi_intervento`.`id` = `in_tipi_intervento_lang`.`id_record` AND `in_tipi_intervento_lang`.`id_lang` = '.prepare(Models\Locale::getDefault()->id).') WHERE `co_promemoria`.`id` = :id', [
    ':id' => $id_record,
]);
