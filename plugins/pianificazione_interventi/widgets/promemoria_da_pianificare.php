...
<div id="promemoria_pianificare_'.$counter.'" '.$attr.'>
    ...
    foreach ($array_promemoria[$mese] as $promemoria) {
        ...
        if ($promemoria->id_sede == '-1') {
            ...
            $rsp2 = $dbo->fetchArray("SELECT id, CONCAT( CONCAT_WS( ' (', CONCAT_WS(', ', nome_sede, citta), indirizzo ), ')') AS descrizione FROM an_sedi WHERE id=".prepare($promemoria->id_sede));
        ...
        echo '
                <td>
                    <button type="button" class="btn btn-primary btn-sm" title="Pianifica intervento ora..." data-widget="tooltip" onclick="launch_modal(\'Pianifica intervento\', \''.base_path_osm().'/add.php?id_module='.Module::where('name', 'Interventi')->first()->id.'&ref=interventi_contratti&id_contratto='.$contratto->id.'&id_contratto_riga='.$promemoria->id.'\');">
