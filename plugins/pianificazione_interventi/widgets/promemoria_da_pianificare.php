...
<div id="promemoria_pianificare_'.$counter.'" '.$attr.'>
    ...
    foreach ($array_promemoria[$mese] as $promemoria) {
        ...
        echo '
                <td>
                    <button type="button" class="btn btn-primary btn-sm" title="Pianifica intervento ora..." data-toggle="tooltip" onclick="launch_modal(\'Pianifica intervento\', \''.base_url().'/add.php?id_module='.Modules::get('Interventi')['id'].'&ref=interventi_contratti&idcontratto='.$contratto->id.'&idcontratto_riga='.$promemoria->id.'\');">
