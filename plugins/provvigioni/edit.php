...
<form action="" method="post" role="form">
	...
	<div class="row">
		...
		<div class="col-md-4">
			{[ "type": "select", "label": "'.tr('Agente').'", "name": "id_agente", "value": "$id_agente$", "values": "query=SELECT `an_anagrafiche`.`id` AS id, CONCAT(`ragione_sociale`, IF(`citta` IS NULL OR `citta` = \'\', \'\', CONCAT(\' (\', `citta`, \')\')), IF(`deleted_at` IS NULL, \'\', \' ('.tr('eliminata').')\')) AS descrizione, `id_tipo_intervento_default` FROM `an_anagrafiche` INNER JOIN (`an_tipi_anagrafiche_anagrafiche` INNER JOIN `an_tipi_anagrafiche` ON `an_tipi_anagrafiche_anagrafiche`.`id_tipo_anagrafica`=`an_tipi_anagrafiche`.`id`LEFT JOIN `an_tipi_anagrafiche_lang` ON (`an_tipi_anagrafiche`.`id`=`an_tipi_anagrafiche_lang`.`id_record` AND `an_tipi_anagrafiche_lang`.`id_lang`='.prepare(Models\Locale::getDefault()->id).')) ON `an_anagrafiche`.`id`=`an_tipi_anagrafiche_anagrafiche`.`id_anagrafica` WHERE `deleted_at` IS NULL AND `title`=\'Agente\' AND `an_anagrafiche`.`id` NOT IN (SELECT `id_agente` FROM `co_provvigioni` WHERE `id_agente`!='.prepare($record['id_agente']).') ORDER BY `ragione_sociale`", "required": 1, "icon-after": "add|'.Module::where('name', 'Anagrafiche')->first()->id.'|tipoanagrafica=Agente&readonly_tipo=1" ]}
		</div>
