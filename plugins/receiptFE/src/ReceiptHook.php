...
class ReceiptHook extends Manager
{
    ...
    public function needsExecution()
    {
        ...
        // Lettura cache
        $todo_cache = Cache::find((new Cache())->getByField('name', 'Ricevute Elettroniche', \Models\Locale::where('predefined', true)->first()->id));
    ...
    public function execute()
    {
        // Lettura cache
        $todo_cache = Cache::find((new Cache())->getByField('name', 'Ricevute Elettroniche', \Models\Locale::where('predefined', true)->first()->id));
        $completed_cache = Cache::find((new Cache())->getByField('name', 'Ricevute Elettroniche importate', \Models\Locale::where('predefined', true)->first()->id));
