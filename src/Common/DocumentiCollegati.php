...
class DocumentiCollegati
{
    ...
    private static function getDocumentiIntervento($id_intervento)
    {
        ...
        $query = 'SELECT 
            ...
            `co_stati_documento_lang`.`title` AS stato_documento,
            `co_tipi_documento`.`dir`
        FROM `co_documenti`
        INNER JOIN `co_tipi_documento` ON `co_tipi_documento`.`id` = `co_documenti`.`id_tipo_documento`
        LEFT JOIN `co_tipi_documento_lang` ON (
            `co_tipi_documento_lang`.`id_record` = `co_documenti`.`id_tipo_documento` AND 
            `co_tipi_documento_lang`.`id_lang` = '.prepare(Locale::getDefault()->id).'
        )
        INNER JOIN `co_stati_documento` ON `co_stati_documento`.`id` = `co_documenti`.`id_stato`
        LEFT JOIN `co_stati_documento_lang` ON (
            `co_stati_documento_lang`.`id_record` = `co_documenti`.`id_stato` AND 
            `co_stati_documento_lang`.`id_lang` = '.prepare(Locale::getDefault()->id).'
        )
    ...
    private static function getDocumentiFatturaVendita($id_fattura)
    {
        ...
        $query_ddt = 'SELECT 
            ...
            IF(`dt_tipi_ddt`.`dir` = \'entrata\', \'Ddt in uscita\', \'Ddt in entrata\') AS modulo,
            `dt_stati_ddt_lang`.`title` AS stato_documento
        FROM `dt_ddt`
        INNER JOIN `co_righe_documenti` ON (
        ...
        LEFT JOIN `dt_tipi_ddt_lang` ON (
            `dt_tipi_ddt_lang`.`id_record` = `dt_ddt`.`id_tipo_ddt` AND 
            `dt_tipi_ddt_lang`.`id_lang` = '.prepare(Locale::getDefault()->id).'
        )
        INNER JOIN `dt_stati_ddt` ON `dt_ddt`.`id_stato` = `dt_stati_ddt`.`id`
        LEFT JOIN `dt_stati_ddt_lang` ON (
            `dt_stati_ddt_lang`.`id_record` = `dt_ddt`.`id_stato` AND 
            `dt_stati_ddt_lang`.`id_lang` = '.prepare(Locale::getDefault()->id).'
        )
        GROUP BY `dt_ddt`.`id`
        ...
        $query_ordini = 'SELECT 
            ...
            IF(`or_tipi_ordine`.`dir` = \'entrata\', \'Ordini cliente\', \'Ordini fornitore\') AS modulo,
        ...
        LEFT JOIN `or_tipi_ordine_lang` ON (
            `or_tipi_ordine_lang`.`id_record` = `or_ordini`.`id_tipo_ordine` AND 
            `or_tipi_ordine_lang`.`id_lang` = '.prepare(Locale::getDefault()->id).'
        )
    ...
    private static function getDocumentiContratto($id_contratto)
    {
        ...
        $query_fatture = 'SELECT
            ...
            `co_documenti`.`data`,
            `co_tipi_documento_lang`.`title` AS tipo_documento,
            IF(`co_tipi_documento`.`dir` = \'entrata\', \'Fatture di vendita\', \'Fatture di acquisto\') AS modulo,
            `co_stati_documento_lang`.`title` AS stato_documento
        FROM `co_documenti`
        INNER JOIN `co_tipi_documento` ON `co_tipi_documento`.`id` = `co_documenti`.`id_tipo_documento`
        LEFT JOIN `co_tipi_documento_lang` ON (
    ...
    private static function getDocumentiPreventivo($id_preventivo)
    {
        ...
        INNER JOIN `co_tipi_documento` ON `co_tipi_documento`.`id` = `co_documenti`.`id_tipo_documento`
        LEFT JOIN `co_tipi_documento_lang` ON (
            `co_tipi_documento_lang`.`id_record` = `co_tipi_documento`.`id` AND
            `co_tipi_documento_lang`.`id_lang` = '.prepare(Locale::getDefault()->id).'
        )
        ...
        LEFT JOIN `co_stati_documento_lang` ON (
            `co_stati_documento`.`id` = `co_stati_documento_lang`.`id_record` AND
            `co_stati_documento_lang`.`id_lang` = '.prepare(Locale::getDefault()->id).'
        )
        ...
        LEFT JOIN `or_tipi_ordine_lang` ON (
            `or_tipi_ordine_lang`.`id_record` = `or_tipi_ordine`.`id` AND
            `or_tipi_ordine_lang`.`id_lang` = '.prepare(Locale::getDefault()->id).'
        )
        ...
        LEFT JOIN `dt_tipi_ddt_lang` ON (
            `dt_tipi_ddt_lang`.`id_record` = `dt_tipi_ddt`.`id` AND
            `dt_tipi_ddt_lang`.`id_lang` = '.prepare(Locale::getDefault()->id).'
        )
        LEFT JOIN `dt_stati_ddt` ON `dt_ddt`.`id_stato` = `dt_stati_ddt`.`id`
        LEFT JOIN `dt_stati_ddt_lang` ON (
            `dt_stati_ddt`.`id` = `dt_stati_ddt_lang`.`id_record` AND
            `dt_stati_ddt_lang`.`id_lang` = '.prepare(Locale::getDefault()->id).'
        )
    ...
    private static function getDocumentiOrdine($id_ordine)
    {
        ...
        LEFT JOIN `co_stati_documento` ON `co_documenti`.`id_stato` = `co_stati_documento`.`id`
        LEFT JOIN `co_stati_documento_lang` ON (
            `co_stati_documento_lang`.`id_record` = `co_stati_documento`.`id` AND
            `co_stati_documento_lang`.`id_lang` = '.prepare(Locale::getDefault()->id).'
        )
        ...
        LEFT JOIN `dt_stati_ddt` ON `dt_ddt`.`id_stato` = `dt_stati_ddt`.`id`
        LEFT JOIN `dt_stati_ddt_lang` ON (
            `dt_stati_ddt_lang`.`id_record` = `dt_stati_ddt`.`id` AND
            `dt_stati_ddt_lang`.`id_lang` = '.prepare(Locale::getDefault()->id).'
        )
    ...
    private static function getInfoDocumento($id, $tipo)
    {
        ...
        if ($tipo == Modules\Interventi\Intervento::class) {
            ...
            $query = 'SELECT
                ...
                `or_stati_ordine_lang`.`title` AS stato_documento
            FROM `'.$tabella.'`
            INNER JOIN `or_tipi_ordine` ON `'.$tabella.'`.`id_tipo_ordine` = `or_tipi_ordine`.`id`
            LEFT JOIN `or_tipi_ordine_lang` ON (
                `or_tipi_ordine_lang`.`id_record` = `or_tipi_ordine`.`id` AND
                `or_tipi_ordine_lang`.`id_lang` = '.prepare(Locale::getDefault()->id).'
            )
            ...
            $query = 'SELECT
                ...
                `dt_stati_ddt_lang`.`title` AS stato_documento
            FROM `'.$tabella.'`
            INNER JOIN `dt_tipi_ddt` ON `'.$tabella.'`.`id_tipo_ddt` = `dt_tipi_ddt`.`id`
            LEFT JOIN `dt_tipi_ddt_lang` ON (
                `dt_tipi_ddt_lang`.`id_record` = `dt_tipi_ddt`.`id` AND
                `dt_tipi_ddt_lang`.`id_lang` = '.prepare(Locale::getDefault()->id).'
            )
            INNER JOIN `dt_stati_ddt` ON `'.$tabella.'`.`id_stato` = `dt_stati_ddt`.`id`
            LEFT JOIN `dt_stati_ddt_lang` ON (
                `dt_stati_ddt_lang`.`id_record` = `dt_stati_ddt`.`id` AND
                `dt_stati_ddt_lang`.`id_lang` = '.prepare(Locale::getDefault()->id).'
            )
            ...
            $query = 'SELECT
                ...
                `co_stati_documento_lang`.`title` AS stato_documento
            FROM `'.$tabella.'`
            INNER JOIN `co_tipi_documento` ON `'.$tabella.'`.`id_tipo_documento` = `co_tipi_documento`.`id`
            LEFT JOIN `co_tipi_documento_lang` ON (
                `co_tipi_documento_lang`.`id_record` = `co_tipi_documento`.`id` AND
                `co_tipi_documento_lang`.`id_lang` = '.prepare(Locale::getDefault()->id).'
            )
            INNER JOIN `co_stati_documento` ON `'.$tabella.'`.`id_stato` = `co_stati_documento`.`id`
            LEFT JOIN `co_stati_documento_lang` ON (
                `co_stati_documento_lang`.`id_record` = `co_stati_documento`.`id` AND
                `co_stati_documento_lang`.`id_lang` = '.prepare(Locale::getDefault()->id).'
            )
