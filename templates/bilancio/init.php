...
$liv2_patrimoniale = $dbo->fetchArray('SELECT co_piano_dei_conti2.numero AS numero, co_piano_dei_conti2.descrizione AS descrizione, SUM(totale) AS totale, co_piano_dei_conti2.id AS id FROM `co_movimenti` INNER JOIN co_piano_dei_conti3 ON co_movimenti.id_conto=co_piano_dei_conti3.id INNER JOIN co_piano_dei_conti2 ON co_piano_dei_conti3.id_piano_dei_conti2=co_piano_dei_conti2.id WHERE id_conto IN(SELECT id FROM co_piano_dei_conti3 WHERE id_piano_dei_conti2 IN(SELECT id FROM co_piano_dei_conti2 WHERE id_piano_dei_conti1=(SELECT id FROM co_piano_dei_conti1 WHERE descrizione="Patrimoniale"))) AND co_movimenti.data>='.prepare($date_start).' AND co_movimenti.data<='.prepare($date_end).' GROUP BY id_piano_dei_conti2 ORDER BY co_piano_dei_conti2.numero');

$liv2_economico = $dbo->fetchArray('SELECT co_piano_dei_conti2.numero AS numero, co_piano_dei_conti2.descrizione AS descrizione, SUM(
    CASE
    ...
    END
) AS totale_reddito, co_piano_dei_conti2.id AS id FROM `co_movimenti` INNER JOIN co_piano_dei_conti3 ON co_movimenti.id_conto=co_piano_dei_conti3.id INNER JOIN co_piano_dei_conti2 ON co_piano_dei_conti3.id_piano_dei_conti2=co_piano_dei_conti2.id WHERE id_conto IN(SELECT id FROM co_piano_dei_conti3 WHERE id_piano_dei_conti2 IN(SELECT id FROM co_piano_dei_conti2 WHERE id_piano_dei_conti1=(SELECT id FROM co_piano_dei_conti1 WHERE descrizione="Economico"))) AND (
    ...
    (data_inizio_competenza IS NOT NULL AND data_fine_competenza IS NOT NULL AND
     data_fine_competenza >= '.prepare($date_start).' AND
     data_fine_competenza <= '.prepare($date_end).')
) GROUP BY id_piano_dei_conti2 ORDER BY co_piano_dei_conti2.numero');

$liv3_patrimoniale = $dbo->fetchArray('SELECT co_piano_dei_conti3.numero AS numero, co_piano_dei_conti3.descrizione AS descrizione, SUM(totale) AS totale, co_piano_dei_conti3.id_piano_dei_conti2 AS id_piano_dei_conti2 FROM `co_movimenti` INNER JOIN co_piano_dei_conti3 ON co_movimenti.id_conto=co_piano_dei_conti3.id INNER JOIN co_piano_dei_conti2 ON co_piano_dei_conti3.id_piano_dei_conti2=co_piano_dei_conti2.id WHERE id_conto IN(SELECT id FROM co_piano_dei_conti3 WHERE id_piano_dei_conti2 IN(SELECT id FROM co_piano_dei_conti2 WHERE id_piano_dei_conti1=(SELECT id FROM co_piano_dei_conti1 WHERE descrizione="Patrimoniale"))) '.(empty(get('elenco_analitico')) ? 'AND co_piano_dei_conti3.id NOT IN (SELECT id_conto_cliente FROM an_anagrafiche) AND co_piano_dei_conti3.id NOT IN (SELECT id_conto_fornitore FROM an_anagrafiche)' : '').' AND co_movimenti.data>='.prepare($date_start).' AND co_movimenti.data<='.prepare($date_end).' GROUP BY id_conto ORDER BY co_piano_dei_conti3.numero');

$liv3_economico = $dbo->fetchArray('SELECT co_piano_dei_conti3.numero AS numero, co_piano_dei_conti3.descrizione AS descrizione, SUM(
    CASE
        WHEN co_movimenti.data BETWEEN '.prepare($date_start).' AND '.prepare($date_end).' THEN
            totale
        ELSE
    ...
    END
) AS totale_reddito, co_piano_dei_conti3.id_piano_dei_conti2 AS id_piano_dei_conti2 FROM `co_movimenti` INNER JOIN co_piano_dei_conti3 ON co_movimenti.id_conto=co_piano_dei_conti3.id INNER JOIN co_piano_dei_conti2 ON co_piano_dei_conti3.id_piano_dei_conti2=co_piano_dei_conti2.id WHERE id_conto IN(SELECT id FROM co_piano_dei_conti3 WHERE id_piano_dei_conti2 IN(SELECT id FROM co_piano_dei_conti2 WHERE id_piano_dei_conti1=(SELECT id FROM co_piano_dei_conti1 WHERE descrizione="Economico"))) AND (
    (co_movimenti.data BETWEEN '.prepare($date_start).' AND '.prepare($date_end).') 
    OR 
    (data_inizio_competenza IS NOT NULL AND data_fine_competenza IS NOT NULL AND 
     data_fine_competenza >= '.prepare($date_start).' AND 
    ...
    (data_inizio_competenza IS NOT NULL AND data_fine_competenza IS NOT NULL AND
     data_fine_competenza >= '.prepare($date_start).' AND
     data_fine_competenza <= '.prepare($date_end).')
) GROUP BY id_conto ORDER BY co_piano_dei_conti3.numero');

$utile_perdita = $dbo->fetchOne('SELECT SUM(totale) AS totale FROM `co_movimenti` WHERE id_conto IN(SELECT id FROM co_piano_dei_conti3 WHERE id_piano_dei_conti2 IN(SELECT id FROM co_piano_dei_conti2 WHERE id_piano_dei_conti1=(SELECT id FROM co_piano_dei_conti1 WHERE descrizione="Economico")))AND co_movimenti.data>='.prepare($date_start).' AND co_movimenti.data<='.prepare($date_end))['totale'];

$debiti_fornitori_attivita = $dbo->fetchArray('
SELECT 
    co_piano_dei_conti3.numero AS numero,
    co_piano_dei_conti3.descrizione AS descrizione,
    SUM(totale) AS totale,
    co_piano_dei_conti3.id_piano_dei_conti2 AS id_piano_dei_conti2
FROM
    `co_movimenti` 
...
WHERE 
    co_piano_dei_conti2.id='.prepare(setting('Conto di secondo livello per i debiti fornitori')).'
    AND co_piano_dei_conti3.id IN (SELECT id_conto_fornitore FROM an_anagrafiche) 
...
FROM
    ...
    INNER JOIN co_piano_dei_conti2 ON co_piano_dei_conti3.id_piano_dei_conti2=co_piano_dei_conti2.id 
WHERE 
    co_piano_dei_conti2.id='.prepare(setting('Conto di secondo livello per i crediti clienti')).'
    AND co_piano_dei_conti3.id IN (SELECT id_conto_cliente FROM an_anagrafiche) 
    AND co_movimenti.data>='.prepare($date_start).' 
    AND co_movimenti.data<='.prepare($date_end).' 
    AND co_movimenti.totale > 0
GROUP BY 
