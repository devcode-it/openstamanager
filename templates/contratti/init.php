...
if (!empty($documento->id_sede_destinazione)) {
    $rsd = $dbo->fetchArray('SELECT (SELECT codice FROM an_anagrafiche WHERE id=an_sedi.id_anagrafica) AS codice, (SELECT ragione_sociale FROM an_anagrafiche WHERE id=an_sedi.id_anagrafica) AS ragione_sociale, nomesede, indirizzo, cap, citta, provincia, p_iva, codice_fiscale, id_nazione FROM an_sedi WHERE id_anagrafica='.prepare($id_cliente).' AND id='.prepare($documento->id_sede_destinazione));
