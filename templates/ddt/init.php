...
$causale = $dbo->fetchOne('SELECT `dt_causale_t`.*, `dt_causale_t_lang`.`title` as descrizione FROM `dt_causale_t` LEFT JOIN `dt_causale_t_lang` ON (`dt_causale_t`.`id` = `dt_causale_t_lang`.`id_record` AND `dt_causale_t_lang`.`id_lang` ='.prepare(Models\Locale::getDefault()->id).') WHERE `dt_causale_t`.`id` = '.prepare($documento['id_causale_t']));
...
$aspetto_beni = $dbo->fetchOne('SELECT `dt_aspetto_beni`.*, `dt_aspetto_beni_lang`.`title` as descrizione FROM `dt_aspetto_beni` LEFT JOIN `dt_aspetto_beni_lang` ON (`dt_aspetto_beni`.`id`=`dt_aspetto_beni_lang`.`id_record` AND `dt_aspetto_beni_lang`.`id_lang`='.prepare(Models\Locale::getDefault()->id).') WHERE `dt_aspetto_beni`.`id` = '.prepare($documento['id_aspetto_beni']));
