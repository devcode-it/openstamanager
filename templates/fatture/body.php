...
$autofill = new Util\Autofill(6, 70, $settings['font-size']);
...
foreach ($righe as $riga) {
    ...
    $aliquota = $database->fetchOne('SELECT percentuale FROM co_iva WHERE id = '.prepare($riga->id_iva))['percentuale'];
...
$diciture[] = $dbo->fetchOne('SELECT `dicitura_fissa_fattura` AS dicitura FROM `an_anagrafiche` WHERE `id` = '.prepare($id_cliente))['dicitura'];
