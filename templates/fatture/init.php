...
$record = $dbo->fetchOne('SELECT 
    ...
    `dt_causale_t_lang`.`title` AS causalet,
...
FROM 
    ...
    LEFT JOIN `dt_causale_t` ON `dt_causale_t`.`id` = `co_documenti`.`id_causale_t`
...
$record['ritenuta_acconto'] = floatval($record['ritenuta_acconto']);
...
$destinazione = '';
if (!empty($record['id_sede_destinazione'])) {
    $rsd = $dbo->fetchArray('SELECT (SELECT `codice` FROM `an_anagrafiche` WHERE `id`=`an_sedi`.`id_anagrafica`) AS codice, (SELECT `ragione_sociale` FROM `an_anagrafiche` WHERE `id`=`an_sedi`.`id_anagrafica`) AS ragione_sociale, `nome_sede`, `indirizzo`, `cap`, `citta`, `provincia`, `p_iva`, `codice_fiscale`, `id_nazione`, `codice_destinatario` FROM `an_sedi` WHERE `id_anagrafica`='.prepare($id_cliente).' AND id='.prepare($record['id_sede_destinazione']));
