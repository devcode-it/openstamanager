...
if (!empty($record['idsede_destinazione'])) {
    $rsd = $dbo->fetchArray('SELECT (SELECT `codice` FROM `an_anagrafiche` WHERE `id`=`an_sedi`.`id_anagrafica`) AS codice, (SELECT `ragione_sociale` FROM `an_anagrafiche` WHERE `id`=`an_sedi`.`id_anagrafica`) AS ragione_sociale, `nomesede`, `indirizzo`, `indirizzo2`, `cap`, `citta`, `provincia`, `p_iva`, `codice_fiscale`, `id_nazione`, `codice_destinatario` FROM `an_sedi` WHERE `id_anagrafica`='.prepare($id_cliente).' AND id='.prepare($record['idsede_destinazione']));
