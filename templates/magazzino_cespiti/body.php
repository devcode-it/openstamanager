...
if (!empty(setting('Magazzino cespiti'))) {
    $query = "SELECT mg_articoli.*, movimenti.qta_totale, `mg_articoli_lang`.`title` AS 'Descrizione', `zz_categorie_lang`.`title` AS 'Categoria' FROM `mg_articoli` LEFT JOIN (SELECT id_articolo, SUM(qta) AS qta_totale FROM mg_movimenti WHERE id_sede=".setting('Magazzino cespiti')." GROUP BY id_articolo) movimenti ON movimenti.id_articolo=mg_articoli.id LEFT JOIN `mg_articoli_lang` ON (`mg_articoli_lang`.`id_record` = `mg_articoli`.`id` AND `mg_articoli_lang`.`id_lang` = '1') LEFT JOIN `zz_categorie` ON `mg_articoli`.`id_categoria` = `zz_categorie`.`id` LEFT JOIN `zz_categorie_lang` ON (`zz_categorie`.`id` = `zz_categorie_lang`.`id_record` AND `zz_categorie_lang`.`id_lang` = '1') WHERE 1=1 AND(`mg_articoli`.`deleted_at`) IS NULL HAVING 2=2 AND movimenti.qta_totale>0 ORDER BY `mg_articoli_lang`.`title`";
