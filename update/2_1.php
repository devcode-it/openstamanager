...
for ($i = 0; $i < sizeof($rs); ++$i) {
    if ($rivalsainps != '') {
        $dbo->query('UPDATE co_righe_documenti SET idrivalsainps='.prepare(get_var('Percentuale rivalsa INPS')).', rivalsainps=( (subtotale-sconto) /100 * 4 ) WHERE iddocumento='.prepare($rs[$i]['id']).'');
    } else {
        $dbo->query('UPDATE co_righe_documenti SET idrivalsainps="0", rivalsainps=0 WHERE iddocumento='.prepare($rs[$i]['id']).'');
    }

    if ($ritenuta != '') {
        $dbo->query('UPDATE co_righe_documenti SET idritenutaacconto='.prepare(get_var("Percentuale ritenuta d'acconto")).', ritenutaacconto=( (subtotale+rivalsainps-sconto) /100 * 20 ) WHERE iddocumento='.prepare($rs[$i]['id']).'');
    } else {
