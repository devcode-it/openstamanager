...
-- Allineamento segmenti
UPDATE `zz_segments` SET `clause` = '`in_interventi`.`id_stato` NOT IN(SELECT `in_statiintervento`.`id` FROM `in_statiintervento` WHERE `is_bloccato`=1)' WHERE `zz_segments`.`name` = 'Non completate' AND `id_module` = (SELECT `id` FROM `zz_modules` WHERE `name` = 'Interventi');
