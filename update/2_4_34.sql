...
UPDATE `zz_views` INNER JOIN `zz_modules` ON `zz_views`.`id_module` = `zz_modules`.`id` SET `zz_views`.`html_format` = 1 WHERE `zz_modules`.`name` = 'Interventi' AND `zz_views`.`name` = 'Sede';
